...
# User parameters
# ------------------------------------------------------------
FLOW_MODEL = "laminar"  # "laminar" (implemented), "spalart-allmaras" (stub)
...
### Reynolds number Re = U * L * rho / mu = 0.2
...
MOVE_LIMIT = 0.2
SNES_LINEAR_SOLVER = "lu"  # use "mumps" if available in your PETSc/FEniCS build
INLET_RAMP_STEPS = 8
...
def build_state_form(flow_model_name, state_u, state_p, adj_u, adj_p, rho_eff, custom_dx):
    ...
    if flow_model_name == "spalart-allmaras":
        raise NotImplementedError(
            "Spalart-Allmaras TO is not implemented in this script yet. "
            "Use a monolithic mixed residual in [u, p, nu_tilde] here, then derive forward/adjoint forms from it."
        )

    raise ValueError(f"Unsupported FLOW_MODEL: {flow_model_name}")
...
state_form = build_state_form(FLOW_MODEL, u, p, v, q, rho_effective, dx)
...
# Output setup
# ------------------------------------------------------------
results_root = os.path.join(THIS_DIR, "PipeBendTO_Results")
