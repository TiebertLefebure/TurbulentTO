...
DESIGN_X_MAX = 1.5 * L
X_MIN = -LEAD_WIDTH
X_MAX = DESIGN_X_MAX + LEAD_WIDTH
Y_MIN = 0.0
Y_MAX = L

PORT_WIDTH = 1.0 / 6.0
BOTTOM_PORT_Y_MIN = 1.0 / 6.0
BOTTOM_PORT_Y_MAX = BOTTOM_PORT_Y_MIN + PORT_WIDTH
TOP_PORT_Y_MAX = 5.0 / 6.0
TOP_PORT_Y_MIN = TOP_PORT_Y_MAX - PORT_WIDTH

U_WALL_RESOLUTION = 1.0
NU = 1.0e-4
RE_WALL = U_WALL_RESOLUTION * PORT_WIDTH / NU
...
def in_top_port(y: float) -> bool:
    return between(y, TOP_PORT_Y_MIN, TOP_PORT_Y_MAX)


def in_bottom_port(y: float) -> bool:
    return between(y, BOTTOM_PORT_Y_MIN, BOTTOM_PORT_Y_MAX)


def in_port(y: float) -> bool:
    return in_top_port(y) or in_bottom_port(y)


def in_design(x: float, y: float) -> bool:
    return between(x, 0.0, DESIGN_X_MAX) and between(y, Y_MIN, Y_MAX)


def in_left_extension(x: float, y: float) -> bool:
    return between(x, X_MIN, 0.0) and in_port(y)


def in_right_extension(x: float, y: float) -> bool:
    return between(x, DESIGN_X_MAX, X_MAX) and in_port(y)
...
def build_mesh() -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    x_coords = clustered_coordinates((X_MIN, 0.0, DESIGN_X_MAX, X_MAX))
