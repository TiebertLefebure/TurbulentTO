...
def convert_msh_to_xdmf(msh_file, output_prefix=""):
    """
    Convert a 2D Gmsh .msh file (v2.2) with triangle and line cells
    into XDMF files for the volume mesh and facet tags.
    """
    try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert a Gmsh .msh file to XDMF for FEniCS.")
    parser.add_argument(
        "msh_file",
        nargs="?",
        default="manifold_dilgen_2d.msh",
        help="Path to the input .msh file (default: manifold_dilgen_2d.msh)",
    )
