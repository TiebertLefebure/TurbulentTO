...
def area_weighted_design_facet_average(
    ...
    facet_area = 0.0
    for marker in marker_list:
        weighted_integral += assemble(avg(field) * interior_facet_measure(marker))
        weighted_integral += assemble(field * boundary_measure(marker))
    ...
    if facet_area <= area_floor:
        return float("nan")
    return float(weighted_integral) / facet_area


def pressure_drop_between_design_facets(
    pressure,
    interior_facet_measure,
    boundary_measure,
    inlet_markers,
    ...
    inlet_pressure = area_weighted_design_facet_average(
        pressure, interior_facet_measure, boundary_measure, inlet_markers,
    )
    outlet_pressure = area_weighted_design_facet_average(
