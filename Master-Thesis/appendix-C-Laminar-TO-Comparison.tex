...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}{X c c c}
		...
		Relative change in $\Delta p$ (Med--Fine) & \multicolumn{3}{c}{[insert]\%} \\
		\hline
	\end{tabularx}
\end{table}
...
\begin{table}[!ht]
	...
	\begin{tabularx}{\textwidth}{X c c}
		\hline
		\textbf{Design Origin} & \textbf{Pressure Drop ($\Delta p$) [Pa]} & \textbf{Relative Performance} \\
		\hline
...
The resulting pressure drops for both optimized geometries are presented in Table~\ref{tab:app_C_doublepipe_perf}, quantifying the performance penalty incurred by neglecting inertial physics during the optimization phase.
...
Evaluating these two disparate geometric strategies under the target turbulent regime yields the pressure drops shown in Table~\ref{tab:app_C_pipebend_perf}. The drastic reduction in form drag highlights the necessity of the fully coupled SA framework for highly convective turning flows.
