...
\begin{equation}
	...
	\bar{S} = \frac{\tilde{\nu}}{\kappa^2 y^2} f_{v2}.
\end{equation}
Here $S$ is the scalar magnitude of the vorticity tensor, $\kappa$ is the von Karman constant, and $y$ is the distance to the nearest wall. During nonlinear iterations, the code evaluates the standard protected form of $\tilde{S}$ recommended in the SA reference implementation \cite{NasaTurb} so that the production term remains well defined when $S+\bar{S}$ becomes small. Small numerical floors are added in the implementation to prevent division by zero. The function $f_{v2}$ is a near-wall modifier:
\begin{equation}
...
\begin{figure}[!ht]
	...
	\caption{Nested Picard-IPCS coupling used by the FEniCS RANS-SA solver. The eddy viscosity $\nu_t$ is assembled from the previous SA working variable $\tilde{\nu}$, the velocity-pressure pair is advanced by the IPCS weak problems, and the updated velocity then drives the next SA transport solve.}
