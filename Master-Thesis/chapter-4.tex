...
\begin{table}[!ht]
	...
	\begin{tabularx}{\textwidth}{X c c c}
		...
		\hline
		Bulk velocity ($U_\mathrm{bulk}$) [m/s] & [TBD] & $18.472$ & [TBD]\% \\
		Friction coefficient ($C_f$) & [TBD] & $5.861\times10^{-3}$ & [TBD]\% \\
...
\begin{equation}
	...
	\right],
\end{equation}
where $p_k$ is the kinematic pressure solved by the IPCS formulation, $\rho=997$ kg/m$^3$, and $|\Gamma|=\int_\Gamma 1\,\mathrm{d}\Gamma$. The inlet and outlet integrals correspond to the marked FEniCS facets \texttt{ds(2)} and \texttt{ds(3)}, respectively, so the reported value is the area-averaged inlet static pressure minus the area-averaged outlet static pressure, converted to pascals. The retained local comparison metrics are the same bend-exit profile discrepancies used for the channel,
\begin{equation}
	E_{\infty} =
...
\begin{equation}
	...
	\right)^{1/2}\times 100\%,
\end{equation}
where $\Gamma_D$ denotes the line segment across the bend-exit diameter and $s$ is the corresponding arclength coordinate. Table~\ref{tab:ubend_errors} summarizes the quantitative error margins and the macroscopic pressure drop ($\Delta p$) across the 2D U-bend domain for both solvers. Since the FEniCS formulation solves for kinematic pressure, its pressure drop is converted to physical static pressure using $\rho=997$ kg/m$^3$ before comparison; equivalently, the fine Ansys pressure drop of $792.995$ Pa corresponds to $\Delta p/\rho = 0.7954$ m$^2$/s$^2$.
...
\begin{table}[!ht]
	...
	\begin{tabularx}{\textwidth}{X c c c}
		...
		\textbf{Metric} & \textbf{Ansys} & \textbf{FEniCS} & \textbf{Rel. diff.} \\
		\hline
		Pressure drop ($\Delta p$) [Pa] & $792.995$ & $[TBD]$ & $TBD\%$ \\
		Max pointwise velocity discrepancy ($E_\infty$) & -- & -- & [TBD]\% \\
		Relative $L_2$ velocity discrepancy ($E_{L_2}$) & -- & -- & [TBD]\% \\
		\hline
	\end{tabularx}
