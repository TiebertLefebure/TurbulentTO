...
\subsection{Sensitivity of the Frozen Adjoint to Forward State Convergence}
A critical aspect of utilizing the frozen turbulence assumption is its absolute reliance on a fully converged forward physical state. Because the frozen adjoint calculates gradient sensitivities based on a static snapshot of the flow field ($\nu_t$), it is highly vulnerable to incomplete forward convergence.

During preliminary testing, it was observed that executing only a single Picard sweep (\texttt{PICARD\_STEPS = 1}) per optimization design update resulted in an "unfinished" flow field being passed to the adjoint solver. Over hundreds of iterative design cycles, the gradient errors generated by this unconverged physical state accumulated, misleading the optimizer and resulting in premature topological artifacts and sub-optimal geometric profiles. 

As detailed in Appendix A, enforcing stricter forward fluid-turbulence coupling by increasing the number of sweeps to \texttt{PICARD\_STEPS = 3} fundamentally altered and improved the final topology. When strictly converged, the frozen topology aligns closely with the true physical optimum predicted by the mathematically coupled (semi-frozen) adjoint. Consequently, a minimum of three Picard iterations was strictly enforced for all benchmark optimizations to ensure the frozen adjoint sensitivities were consistently derived from a fully equilibrated physical state.
