...
The assessment strategy is deliberately separated into three stages. First, the predictive scope of the SA model for curvature-dominated internal flows is established and benchmarked against Ansys Fluent. Second, the custom SA implementation in FEniCS is verified on turbulent benchmark flows. Third, the Alexandersen pipe-bend optimization is verified through finite-difference sensitivity checks and exported to Ansys Fluent for sharp-geometry validation. This final stage compares FEniCS-SA with Ansys-SA on the same geometry and then evaluates the same design with SST $k$-$\omega$ to quantify turbulence-model sensitivity. In this revised framing, laminar-versus-turbulent design comparison is retained as supporting context rather than treated as CFD validation in itself.
