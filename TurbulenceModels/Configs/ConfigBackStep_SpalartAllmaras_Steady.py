...
# Verification runs should start from the configured initial condition instead
# of silently picking up an older checkpoint from a different stabilization run.
...
FLUID_DENSITY = 1.0  # kg/m^3; used to convert kinematic pressure to static pressure.
PRESSURE_UNIT = "Pa"
BODY_FORCE = (0.0, 0.0)
...
# Checkpoints are still written for inspection, but this verification setup is
# configured as a fresh run by default via RESTART_FROM_SAVED_STATE = False.
...
# Try the reciprocal relaxed-wall solve on the Medium backstep mesh.
...
WALL_DISTANCE_YOON_NEWTON_RTOL = 1.0e-6
WALL_DISTANCE_YOON_NEWTON_ATOL = 1.0e-10
WALL_DISTANCE_YOON_NEWTON_MAX_ITERATIONS = 250
WALL_DISTANCE_YOON_NEWTON_RELAXATION = 0.1
WALL_DISTANCE_YOON_NEWTON_REPORT = True
