...
from Configs.ConfigPipeBend_Laminar import *
import time
...
for iter in range(simulation_prm['MAX_ITERATIONS']):
    ...
    break_flag, errors = are_close_all([u1, p1], [u0, p0], simulation_prm['TOLERANCE'])
    print(
        f'iter: {iter + 1} ({time.time() - start_time:.2f}s, dt = {float(dt):.2e}s) --- L2 errors: '
