...
class KEpsilonTransient(KEpsilonGeneral):
    ...
    def construct_forms(self, external_u1):
        ...
        # SUPG Stabilization
        mesh = self._k.function_space().mesh()
        h = CellDiameter(mesh)
        u_mag = sqrt(dot(external_u1, external_u1) + 1e-10)
        tau = h / (2.0 * u_mag)

        # Residuals for SUPG
        res_k = (self._k - self._k0) / self._dt + dot(external_u1, nabla_grad(self._k)) \
              - self._prod_k + self._react_k * self._k
        res_e = (self._e - self._e0) / self._dt + dot(external_u1, nabla_grad(self._e)) \
              - self._prod_e + self._react_e * self._e

        F_supg_k = inner(tau * dot(external_u1, nabla_grad(self._phi)), res_k) * self._dx
        F_supg_e = inner(tau * dot(external_u1, nabla_grad(self._psi)), res_e) * self._dx
        ...
        FK  = dot((self._k - self._k0) / self._dt, self._phi)*self._dx \
            ...
            + inner((self._nu + self._nu_t / 1.0) * grad(self._k), grad(self._phi))*self._dx \
            - dot(self._prod_k, self._phi)*self._dx \
            + dot(self._react_k * self._k, self._phi)*self._dx + F_supg_k
        self._a_k = lhs(FK); self._l_k = rhs(FK)
