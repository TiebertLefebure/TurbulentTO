...
alpha_fluid = Constant(2.5 * MU_FLUID_VALUE / 100.0**2.0) # alpha_fluid = 
...
if use_outlet_pressure_bc and use_pressure_pin:
    print("Outlet pressure BC requested; disabling the redundant pointwise pressure pin.")
...
if use_outlet_pressure_bc:
    print("Outlet BC type: pressure (p = {:.3e} on outlet).".format(outlet_pressure_value_float))
else:
    print("Outlet BC type: velocity profile.")
...
if callable(custom_turbulence_inlet_builder):
    ...
    print("Using custom SA inlet profiles from config (nu_lam = {:.3e}).".format(_nu_lam))
else:
    ...
    print("SA inlet nu_tilde: {}  (nu_lam = {:.3e})".format(
...
if use_penalized_wall_distance:
    ...
    print("Penalized SA wall-distance enabled: sigma={}, G0={}, alpha_G={}, n_G={}".format(
        ...
        mass_flow_constraints.append(
            {
                ...
                "adjoint_form": derivative(
                    constraint_lagrangian_form, w_fwd, TestFunction(FlowSpaceAdj)
                ),
...
def solve_forward(solve_label=None):
    ...
    for attempt_idx in range(max_restarts + 1):
        ...
        attempt_summaries.append(
            "attempt {}: dt={:.2e}, du={:.2e} (target {:.2e}), dp={:.2e} (target {:.2e})".format(
                attempt_idx + 1, current_dt, du_rel, rtol_u, dp_rel, rtol_p,
            )
        )
        if converged:
        ...
        # Keep the best iterate reached so far and continue from it if a
        # smaller dt / stronger relaxation is needed on the next attempt.
        assign(u_start, u_pc_old)
        assign(p_start, p_pc_old)
    ...
    assign(w_fwd.sub(0), u_pc_old)
    assign(w_fwd.sub(1), p_pc_old)
    message = "IPCS did not meet the steady-state tolerances after {} attempt(s): {}".format(
        max_restarts + 1, "; ".join(attempt_summaries),
    )
