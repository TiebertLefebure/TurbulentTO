...
if not sa_wall_model:
    if callable(custom_wall_distance_updater_builder):
        sa_wall_model = "custom_updater"
    elif use_penalized_wall_distance:
...
valid_sa_wall_models = {"custom_updater", "custom_initial", "direct_y", "geometric", "penalized_g"}
...
if sa_wall_model == "custom_updater":
    if not callable(custom_wall_distance_updater_builder):
        raise ValueError("SA_WALL_MODEL='custom_updater' requires build_wall_distance_updater in the config.")
    wall_distance, update_wall_distance_field = custom_wall_distance_updater_builder(
        TurbulenceSpace,
        DensitySpace,
        mesh,
        boundaries,
    ...
    root_print("Custom SA wall-distance updater enabled from config.")
elif sa_wall_model == "direct_y":
    ...
    wall_distance, update_wall_distance_field = build_direct_wall_distance_solver(
        TurbulenceSpace,
        boundaries,
        wall_markers,
        dx,
    ...
    root_print(
        "Direct SA wall-distance enabled (penalized y eikonal): source={}, init_y={}, eta_y={}, eps_y={}, alpha_y={}, n_y={}, rho_cut_y={}, relax_y={}, maxit_y={}".format(
...
def build_state_snes_recovery_attempts():
    base_attempt = {
        "label": "primary solve",
        "method": str(globals().get("FULL_STATE_SNES_METHOD", "newtonls")),
        "line_search": str(globals().get("FULL_STATE_SNES_LINE_SEARCH", "bt")),
        "rtol": float(globals().get("FULL_STATE_SNES_RTOL", globals().get("FORWARD_SNES_RTOL", 1.0e-6))),
        "atol": float(globals().get("FULL_STATE_SNES_ATOL", globals().get("FORWARD_SNES_ATOL", 1.0e-8))),
        "max_iters": int(globals().get("FULL_STATE_SNES_MAX_ITERS", globals().get("SNES_MAX_ITERS", 80))),
        "restart_with_stokes": False,
    }
    configured_attempts = globals().get("FULL_STATE_SNES_RECOVERY_ATTEMPTS")
    if configured_attempts is None:
        fallback_attempt = dict(base_attempt)
        fallback_attempt.update(
            {
                "label": "fallback retry",
                "method": str(globals().get("FULL_STATE_SNES_FALLBACK_METHOD", base_attempt["method"])),
                "line_search": str(
                    globals().get("FULL_STATE_SNES_FALLBACK_LINE_SEARCH", base_attempt["line_search"])
                ),
                "max_iters": int(
                    globals().get("FULL_STATE_SNES_FALLBACK_MAX_ITERS", base_attempt["max_iters"])
                ),
                "restart_with_stokes": bool(globals().get("FULL_STATE_RESTART_WITH_STOKES", True)),
            }
        )
    ...
    for attempt_idx, attempt_spec in enumerate(configured_attempts, start=1):
        if not isinstance(attempt_spec, dict):
            raise TypeError("FULL_STATE_SNES_RECOVERY_ATTEMPTS entries must be dictionaries.")
